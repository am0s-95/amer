...
description: Deep video analysis fallback (claude-video-vision MCP). Do NOT use as the first choice for videos — try the /watch skill first. Use this skill only when /watch has failed after 1-2 attempts (download error, no usable captions, broken frames), or when the task needs structural analysis /watch cannot do (scene/silence/motion detection, per-segment variable FPS, drill-down into specific segments). Higher token cost than /watch.
---
