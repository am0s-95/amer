...
/* eslint-disable */
/**
 * analyze-sessions.mjs (Skill Master curated fork of Anthropic's session-report)
 *
 * Scans Claude Code transcript files and reports token usage, message counts,
 * runtime, cache breaks, subagent and skill activity. Local-only: reads
 * transcripts from disk and writes to stdout — no network calls.
 *
 * Output is human-readable text by default; pass --json for machine-readable.
 *
 * Root directory resolution (highest priority first):
 *   1. --dir <path>              explicit override
 *   2. $CLAUDE_CONFIG_DIR/projects
 *   3. ~/.claude/projects        (default)
 *
 * Privacy: prompt text and its surrounding transcript context are redacted
 * by default (top_prompts[].text = "[redacted]", context = null). Pass
 * --include-prompts to embed the original text — this also embeds
 * conversation content in the resulting report, local-only, never sent
 * over the network by this script.
 *
 * Usage:
 *   node analyze-sessions.mjs [--dir <projects-dir>] [--json] [--since <ISO|7d|24h|30d|all>] [--top N] [--include-prompts]
 *
...
const ROOT = flag('--dir', path.join(CONFIG_DIR, 'projects'))
const AS_JSON = argv.includes('--json')
const TOP_N = parseInt(flag('--top', '15'), 10)
const SINCE_RAW = flag('--since', null)
...
const INCLUDE_PROMPTS = argv.includes('--include-prompts')

if (INCLUDE_PROMPTS) {
  process.stderr.write(
    '⚠ --include-prompts: the report will contain verbatim text from your ' +
      'conversations (prompts and surrounding transcript context). Treat the ' +
...
if (!fs.existsSync(ROOT)) {
  process.stderr.write(
    `note: no transcripts directory found at ${ROOT} — reporting an empty result.\n`,
  )
}
...
function printJson({ overall, perProject, perSubagent, perSkill }) {
  const out = {
    ...
    requested_since: SINCE_RAW,
    since: SINCE_RAW, // kept for template.html back-compat (same value)
    privacy: {
    ...
    cache_breaks: overall.cacheBreaks
      ...
      .map(({ prompt, ...b }) => ({
        ...b,
        context: INCLUDE_PROMPTS && prompt ? buildContext(prompt) : null,
      })),
...
function topPrompts(n) {
  return [...prompts.entries()]
    ...
    .map(([pk, r]) => ({
      ...
      project: r.project,
      session: r.sessionId,
      text: INCLUDE_PROMPTS ? r.text : '[redacted]',
      ...
      context: INCLUDE_PROMPTS ? buildContext(pk) : null,
    }))
}
...
function printText({ overall, perProject, perSubagent, perSkill }) {
  ...
  line(
    `(prompts ${INCLUDE_PROMPTS ? 'included — treat this output as sensitive' : 'redacted (pass --include-prompts to embed them)'})`,
  )
  hr()
