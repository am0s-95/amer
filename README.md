...
المثبّت idempotent ويقوم بعشرة أشياء:
...
9. **Session Report Curated plugin**: يثبّت عالميًا في نفس Marketplace المحلي (`skill-master-plugins`) عنصر Plugin رابع — نسخة منقّحة محليًا (لا `git-subdir` حي، بل ملفات منسوخة ومعدَّلة داخل هذا المستودع) من سكيل `session-report` الرسمي من Anthropic ([anthropics/claude-plugins-official](https://github.com/anthropics/claude-plugins-official)، مجلد `plugins/session-report`، commit ثابت `b4810bd800e10c8595d79835e61e5945c1cd81ba`). يولّد تقرير HTML **محليًا بالكامل** (بلا شبكة إطلاقًا) عن استهلاك Claude Code — tokens، cache، subagents، skills، أغلى البرومبتات — من transcripts المتاحة داخل `HOME`/`CLAUDE_CONFIG_DIR` الحالي فقط، عبر `claude plugin install session-report-curated@skill-master-plugins --scope user`. التعديلان الجوهريان على المصدر الرسمي: (1) إضافة `.claude-plugin/plugin.json` صريح (المصدر الرسمي يفتقده — قد يسبب namespace مكررًا باسم Git SHA)، و(2) إصلاح `analyze-sessions.mjs` ليحترم `$CLAUDE_CONFIG_DIR` بدل تثبيت المسار على `~/.claude/projects` (مع بقاء `--dir` بأولوية كاملة). **الخصوصية افتراضيًا (بلا أي flag)**: نص البرومبتات وسياقها المحيط مخفيان تمامًا (`text: "[redacted]"`, `context: null`) — لا يظهر محتوى محادثات المستخدم في HTML أو JSON إلا بتمرير `--include-prompts` صراحة (يطبع تحذيرًا واضحًا عندها). التقرير يُحفظ افتراضيًا في `~/.claude/reports/` (صلاحيات `700`/`600`)، لا داخل مستودع المشروع. **لا يشمل** أي Hooks أو agents أو MCP servers أو LSP servers، ولا يُنشئ تقريرًا أو يقرأ transcripts أثناء التثبيت نفسه. الإصدار الرسمي غير المنقّح (`session-report@claude-plugins-official`) **لا يُثبَّت هنا إطلاقًا**. راجع `PROVENANCE.md` لتفاصيل الإصلاحين ونتائج الاختبارات. يمكن تخطي هذا الجزء بمتغيّر البيئة `SESSION_REPORT_CURATED_SKIP_INSTALL=1`.
...
bash tests/test-install-session-report-curated.sh  # Session Report Curated: صحة marketplace.json/plugin.json (مصدر محلي، سكيل واحدة فقط، بلا hooks/agents/mcpServers/lspServers)، الـidempotency، بقاء Ponytail/Obsidian/MCP Server Dev Curated، عدم تثبيت session-report الرسمي بالتوازي، قواعد Session report routing (claude CLI وهمي — بلا شبكة)
bash tests/test-session-report-analyzer.sh  # analyze-sessions.mjs: CLAUDE_CONFIG_DIR، أولوية --dir، مجلد مفقود بأمان، 24h/7d/30d/all، إخفاء prompts افتراضيًا، --include-prompts، dedup، لا تسرّب نص سرّي، HTML بلا CDN (fixtures صناعية — بلا transcripts حقيقية)
```
